...
const responseSchema = {
    ...
    properties: {
        ...
        category: {
            type: "string",
            description: "할 일 종류 (예: ToDo, Note, Work 중 하나로 분류)",
        },
        dueDate: {
            type: "string",
        ...
        priority: {
            type: "string",
            description: "할 일의 중요도 또는 우선 순위 (예: High, Medium, Low)",
        },
        isCompleted: {
            type: "boolean",
...
const systemInstruction = `
    ...
    2. 사용자가 '오늘', '내일', '다음 주'와 같이 **상대적인 날짜**를 언급하면, 이 **현재 날짜(${todayDate})를 기준으로 정확한 'YYYY-MM-DD' 형식으로 변환하여 'dueDate' 필드에 넣어주세요.**
    3. 할 일이나 업무 관련 내용만 처리하며, 그 외의 일반적인 대화, 인사, 질문은 무시하고 **"isMemo": false**로 설정합니다.
    4. 응답은 오직 JSON 형식이며, JSON 스키마를 엄격하게 준수합니다.
    5. 날짜 정보가 명확히 없다면 'dueDate'는 빈 문자열("") 또는 null로 처리합니다.
